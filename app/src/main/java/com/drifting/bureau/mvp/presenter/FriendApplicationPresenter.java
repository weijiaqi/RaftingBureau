 ...
 */
@ActivityScope
public class FriendApplicationPresenter extends BasePresenter<FriendApplicationContract.Model, FriendApplicationContract.View>{
    @Inject
    ...
    @Inject
    public FriendApplicationPresenter (FriendApplicationContract.Model model, FriendApplicationContract.View rootView) {
     ...
     * 好友申请列表（消息中心）
     */
    public void applyToMe(int page, int limit, boolean loadType) {
        if (mRootView != null) {
            mRootView.onloadStart();
        }
        mModel.applyToMe(page, limit)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .compose(RxLifecycleUtils.bindToLifecycle(mRootView))
                .subscribe(new ErrorHandleSubscriber<BaseEntity<FriendApplicationEntity>>(mErrorHandler) {
                    @Override
                    public void onNext(BaseEntity<FriendApplicationEntity> baseEntity) {
                        if (mRootView != null) {
                            if (baseEntity.getCode() == 200) {
                                ...
                                mRootView.onApplyToMeSuccess(baseEntity.getData(), loadType);
                            } else {
