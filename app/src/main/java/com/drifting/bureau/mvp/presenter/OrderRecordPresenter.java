...
public class OrderRecordPresenter extends BasePresenter<OrderRecordContract.Model, OrderRecordContract.View>{
     ...
     * 订单记录（关于我）
     */
    public void ordermine(int page, int limit, boolean loadType) {
        if (mRootView != null) {
            mRootView.onloadStart();
        }
        mModel.ordermine(page, limit)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .compose(RxLifecycleUtils.bindToLifecycle(mRootView))
                .subscribe(new ErrorHandleSubscriber<BaseEntity<OrderRecordEntity>>(mErrorHandler) {
                    @Override
                    public void onNext(BaseEntity<OrderRecordEntity> baseEntity) {
                        if (mRootView != null) {
                            if (baseEntity.getCode() == 200) {
                                ...
                                mRootView.onOrderMineSuccess(baseEntity.getData(), loadType);
                            } else {
