 ...
 */
@ActivityScope
public class DriftingTrackPresenter extends BasePresenter<DriftingTrackContract.Model, DriftingTrackContract.View>{
    @Inject
    ...
    @Inject
    public DriftingTrackPresenter (DriftingTrackContract.Model model, DriftingTrackContract.View rootView) {
     ...
     * 漂流轨迹
     */
    public void messagemine(int page, int limit, boolean loadType) {
        if (mRootView != null) {
            mRootView.onloadStart();
        }
        mModel.messagemine(page, limit)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .compose(RxLifecycleUtils.bindToLifecycle(mRootView))
                .subscribe(new ErrorHandleSubscriber<BaseEntity<DriftingTrackEntity>>(mErrorHandler) {
                    @Override
                    public void onNext(BaseEntity<DriftingTrackEntity> baseEntity) {
                        if (mRootView != null) {
                            if (baseEntity.getCode() == 200) {
                                ...
                                mRootView.myMessageMineSuccess(baseEntity.getData(), loadType);
                            } else {
